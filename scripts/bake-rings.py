...
# Second, optional price source: the open-data mini-lake built by
# scripts/build-open-lake.py from the Kenneth R. French Data Library. It is
# gitignored and regenerated by the builder (locally and in CI); when it is
# absent the bake simply produces the lake-only forest.
OPEN_LAKE = os.path.abspath(os.environ.get("OPEN_LAKE", os.path.join(REPO, "openlake", "data")))
...
# ---- price sources ----------------------------------------------------------
# The forest is baked from one or more lakes laid out in the same schema
# (prices/<id>.parquet + groups.json + fundamentals/meta.parquet). `floor` is
# the source's licensing history floor, or None where the data is free to
# publish in full.

class Source:
    def __init__(self, name, root, floor):
        self.name = name
        self.root = root
        self.prices = os.path.join(root, "prices")
        self.floor = floor
...
def load_sources():
    sources = [Source("lake", LAKE, LAKE_HISTORY_FLOOR)]
    if os.path.isdir(os.path.join(OPEN_LAKE, "prices")):
        sources.append(Source("open", OPEN_LAKE, None))
    else:
        print(f"note: no open-data lake at {OPEN_LAKE} — "
              "run scripts/build-open-lake.py to include the century trees")
    return sources


def load_asset_names(sources):
    """Merge each source's equity names into the cross-asset display-name map."""
    names = dict(DISPLAY_NAMES)
    for src in sources:
        meta_path = os.path.join(src.root, "fundamentals", "meta.parquet")
...
def load_class_map(sources):
    tk2cls = {}
    for src in sources:
        with open(os.path.join(src.root, "groups.json")) as f:
            g = json.load(f)
...
def main():
    ...
    tk2cls = load_class_map(sources)
    asset_names = load_asset_names(sources)
