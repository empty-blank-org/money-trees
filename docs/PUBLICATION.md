...
- [ ] Add `moneytreerings.com` (registered at Hover, 2026-09-18) to Cloudflare and attach it to the Pages project as the production custom domain. `moneytrees.fun` belongs to an unrelated crypto token project — never point anything at it.
- [ ] Redirect `www.moneytreerings.com` to `https://moneytreerings.com` (or deliberately choose the inverse) so there is one canonical host.
- [ ] Confirm Cloudflare has issued the edge TLS certificate and `https://moneytreerings.com` resolves before announcing it.
- [ ] Keep the production `SITE_URL` at its default `https://moneytreerings.com`; override it only for an intentionally separate preview environment.
